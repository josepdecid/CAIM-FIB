...
if __name__ == '__main__':
    param_grid = {
        # ExtractData
        'minfreq': np.arange(0.0, 0.3, step=0.3, dtype=float).tolist(),
        'maxfreq': np.arange(0.4, 1.0, step=1, dtype=float).tolist(),
        'numwords': [100],
        # GeneratePrototypes
        'nclust': np.arange(2, 4, dtype=int).tolist(),
        # MRKmeans
        'iter': np.arange(5, 15, step=5, dtype=int).tolist(),
        'nmaps': np.arange(2, 4, dtype=int).tolist(),
        'nreduces': np.arange(2, 4, dtype=int).tolist(),
        'sim': ['jaccard', 'cosine']
    }
    ...
    for params in grid:
        ...
        extract_data_params = f'--index arxivs --minfreq {params["minfreq"]} --maxfreq {params["maxfreq"]} ' \
                              f'--numwords {params["numwords"]} --folder {folder_name}'
        system(f'python ExtractData.py {extract_data_params}')
        print(f'Data extracted for {extract_data_params}')

        prototype_params = f'--nclust {params["nclust"]} --folder {folder_name}'
